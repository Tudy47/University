...
public class Bettelmann {
    ...
    public void playRound() {
        ...
        List<Card> openPile = new ArrayList<>();
        boolean war = true;

        while (war) {
            // Dacă un jucător nu mai are cărți, celălalt câștigă
            ...
            // Fiecare jucător trage o carte
            Card card1 = closedPile1.removeFirst();
            Card card2 = closedPile2.removeFirst();
            ...
            if (comparison > 0) {
                // Jucătorul 1 câștigă runda
                for (Card c : openPile) {
                    closedPile1.addLast(c);
                }
                war = false;
            } else if (comparison < 0) {
                // Jucătorul 2 câștigă runda
                for (Card c : openPile) {
                    closedPile2.addLast(c);
                }
                war = false;
            } else {
                // Război: fiecare jucător pune o carte cu fața în jos
                if (closedPile1.isEmpty()) {
                    winner = 2;
                    return;
                }
                ...
                openPile.add(closedPile1.removeFirst());
                openPile.add(closedPile2.removeFirst());
                // continuă bucla war = true
            }
        }

        // După rundă, verificăm dacă cineva a rămas fără cărți
        if (closedPile1.isEmpty()) {
            winner = 2;
        } else if (closedPile2.isEmpty()) {
            winner = 1;
        } else {
            winner = -1; // jocul continuă
        }
