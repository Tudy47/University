...
public class RicochetRobots {
    ...
    public static PartialSolution bfsWithHashing(Board board) {
        /* TODO */
        LinkedList<PartialSolution> coada = new LinkedList<>();
        HashSet<Board> vizitate = new HashSet<>();
        coada.add(new PartialSolution(board));
        vizitate.add(board);
        while (!coada.isEmpty()) {
            PartialSolution curent = coada.removeFirst();

            for (int i = 0; i < mutari.size(); i++) {
                Move m = mutari.get(i);

                if (!vizitate.contains(urmator)) {
                    vizitate.add(urmator);
                    lista.add(m);
                    coada.add(new PartialSolution(urmator, lista, m));
                }
            }
        }
        return null;
